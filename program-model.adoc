...
The read-only XLEN-wide _matrix_ _type_ CSR, `mtype`, provides the default type used to interpret the contents of the matrix register file, and can only be updated by `msettype{i}` and field-set instructions. The matrix type determines the organization of elements in each matrix register.
