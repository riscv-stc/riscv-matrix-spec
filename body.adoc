...
Examples:
    ...
    msettypei t0, e64, fp64          # SEW = 8, support 64-bit float point
```
...
```
mlre8.m  md, (rs1), rs2, lmul   #  8-bit whole matrix load
mlre16.m md, (rs1), rs2, lmul   # 16-bit whole matrix load
mlre32.m md, (rs1), rs2, lmul   # 32-bit whole matrix load
mlre64.m md, (rs1), rs2, lmul   # 64-bit whole matrix load
```
...
mfncvt.f.fw.m  td, ts1, ts2, lmul   # double-width float to single-width float
mfwcvt.fw.f.m  td, ts1, ts2, lmul   # single-width float to double-width float

# Convert integer to float
mfcvt.f.x.m    td, ts1, ts2, lmul   # integer to float

mfncvt.f.xw.m  td, ts1, ts2, lmul   # double-width integer to float
mfncvt.f.xq.m  td, ts1, ts2, lmul   # quad-width integer to float

mfwcvt.fw.x.m  td, ts1, ts2, lmul   # single-width integer to double-width float
mfcvt.fw.xw.m  td, ts1, ts2, lmul   # double-width integer to double-width float
mfncvt.fw.xq.m td, ts1, ts2, lmul   # quad-width integer to double-width float

# Convert float to integer
mfcvt.x.f.m    td, ts1, ts2, lmul   # float to integer

mfwcvt.xw.f.m  td, ts1, ts2, lmul   # float to double-width integer
mfwcvt.xq.f.m  td, ts1, ts2, lmul   # float to quad-width integer

mfncvt.x.fw.m  td, ts1, ts2, lmul   # double-width float to single-width integer
mfcvt.xw.fw.m  td, ts1, ts2, lmul   # double-width float to double-width integer
mfwcvt.xq.fw.m td, ts1, ts2, lmul   # double-width float to quad-width integer
```
...
|===
| No.  |                 | **31 26** | 25   | 24 20 | 19 15 | 14  12 | 11 10 | 9 7 | 6    0
2+|**Convert**          ^| funct6    | fdst | ts2   | ts1   | funct3 | lmul  | td  | opcode
| 1    | mfncvtc.f.fw.m  | 010000    | 1    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 2    | mfwcvtc.fw.f.m  | 010000    | 0    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 3    | mfcvtc.f.x.m    | 010010    | 1    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 4    | mfcvtc.x.f.m    | 010010    | 0    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 5    | mfncvtc.f.xw.m  | 010011    | 1    | ts2   | ts1   | 110    | lmul  | td  | 1110111 
| 6    | mfwcvtc.xw.f.m  | 010011    | 0    | ts2   | ts1   | 110    | lmul  | td  | 1110111 
| 7    | mfncvtc.f.xq.m  | 010100    | 1    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 8    | mfwcvtc.xq.f.m  | 010100    | 0    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 9    | mfwcvtc.fw.x.m  | 010101    | 1    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 10   | mfncvtc.x.fw.m  | 010101    | 0    | ts2   | ts1   | 110    | lmul  | td  | 1110111
| 11   | mfcvtc.fw.xw.m  | 010110    | 1    | ts2   | ts1   | 110    | lmul  | td  | 1110111 
| 12   | mfcvtc.xw.fw.m  | 010110    | 0    | ts2   | ts1   | 110    | lmul  | td  | 1110111 
| 13   | mfncvtc.fw.xq.m | 010111    | 1    | ts2   | ts1   | 110    | lmul  | td  | 1110111 
| 14   | mfwcvtc.xq.fw.m | 010111    | 0    | ts2   | ts1   | 110    | lmul  | td  | 1110111 
|===
